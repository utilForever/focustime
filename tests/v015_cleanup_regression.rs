...
fn v015_cleanup_docs_keep_matrix_and_release_guidance_aligned() {
    ...
    for required in [
        ...
        "Blocklist category config remains deprecated with config-doctor migration guidance",
        "Deprecated blocklist category config migrates into profile-level `sites` and `allowlist_sites`",
    ...
    for required in [
        ...
        "Blocklist category config (`blocklist_profiles.categories`, `selected_category`)",
