...
fn sync_backup_and_restore_json_round_trip_config_and_stats_files() {
    ...
    let source_sync_dir = source_env.root.join("sync-bundle");
    let target_sync_dir = target_env.root.join("sync-bundle");
    let backup_arg = format!("--sync-backup={}", source_sync_dir.display());
    let restore_arg = format!("--sync-restore={}", target_sync_dir.display());

    let set_goal_output = source_env.run(&["--goal=120,4", "--json"]);
    assert_eq!(set_goal_output.status.code(), Some(0));
    assert!(stderr_text(&set_goal_output).trim().is_empty());

    let set_task_goal_output = source_env.run(&["--task-goal=Docs:90,3", "--json"]);
    assert_eq!(set_task_goal_output.status.code(), Some(0));
    assert!(stderr_text(&set_task_goal_output).trim().is_empty());

    let backup_output =
        source_env.run(&[backup_arg.as_str(), "--sync-passphrase=secret", "--json"]);
    assert_eq!(backup_output.status.code(), Some(0));
    assert!(stderr_text(&backup_output).trim().is_empty());
    let backup_payload: Value =
        serde_json::from_slice(&backup_output.stdout).expect("stdout should be JSON");
    assert!(backup_payload.get("bundle_dir").is_some());
    assert!(backup_payload.get("bundle_path").is_some());
    assert!(backup_payload.get("snapshot_id").is_some());
    assert!(source_sync_dir.join("focustime-sync.toml").is_file());

    fs::create_dir_all(&target_sync_dir).expect("failed to create target sync directory");
    fs::copy(
        source_sync_dir.join("focustime-sync.toml"),
        target_sync_dir.join("focustime-sync.toml"),
    )
    .expect("failed to copy sync bundle to target env");

    let mutate_goal_output = target_env.run(&["--goal=15,1", "--json"]);
    assert_eq!(mutate_goal_output.status.code(), Some(0));
    assert!(stderr_text(&mutate_goal_output).trim().is_empty());

    let mutate_task_goal_output = target_env.run(&["--task-goal=Docs:30,1", "--json"]);
    assert_eq!(mutate_task_goal_output.status.code(), Some(0));
    assert!(stderr_text(&mutate_task_goal_output).trim().is_empty());

    let restore_output =
        target_env.run(&[restore_arg.as_str(), "--sync-passphrase=secret", "--json"]);
    assert_eq!(restore_output.status.code(), Some(0));
    assert!(stderr_text(&restore_output).trim().is_empty());
    let restore_payload: Value =
        serde_json::from_slice(&restore_output.stdout).expect("stdout should be JSON");
    assert!(restore_payload.get("restore_dir").is_some());
    assert!(restore_payload.get("bundle_path").is_some());
    assert!(restore_payload.get("snapshot_id").is_some());
    assert!(restore_payload.get("config_restored_path").is_some());
    assert!(restore_payload.get("stats_restored_path").is_some());

    let goal_output = target_env.run(&["--goal", "--json"]);
    assert_eq!(goal_output.status.code(), Some(0));
    let goal_payload: Value = serde_json::from_slice(&goal_output.stdout).unwrap();
    assert_eq!(goal_payload["minutes_target"], 120);
    assert_eq!(goal_payload["pomodoros_target"], 4);

    let task_goal_output = target_env.run(&["--task-goal", "Docs", "--json"]);
    assert_eq!(task_goal_output.status.code(), Some(0));
    let task_goal_payload: Value = serde_json::from_slice(&task_goal_output.stdout).unwrap();
    assert_eq!(task_goal_payload["minutes_target"], 90);
    assert_eq!(task_goal_payload["pomodoros_target"], 3);
...
#[test]
fn sync_restore_json_fails_with_wrong_passphrase() {
    let env = TestEnv::new("sync-restore-wrong-passphrase");
    let sync_dir = env.root.join("sync-bundle");
    let backup_arg = format!("--sync-backup={}", sync_dir.display());
    let restore_arg = format!("--sync-restore={}", sync_dir.display());

    let seed_goal_output = env.run(&["--goal=120,4", "--json"]);
    assert_eq!(seed_goal_output.status.code(), Some(0));
    assert!(stderr_text(&seed_goal_output).trim().is_empty());
    let seed_task_goal_output = env.run(&["--task-goal=Docs:90,3", "--json"]);
    assert_eq!(seed_task_goal_output.status.code(), Some(0));
    assert!(stderr_text(&seed_task_goal_output).trim().is_empty());

    let backup_output = env.run(&[backup_arg.as_str(), "--sync-passphrase=secret", "--json"]);
    assert_eq!(backup_output.status.code(), Some(0));
    assert!(stderr_text(&backup_output).trim().is_empty());

    let restore_output = env.run(&[restore_arg.as_str(), "--sync-passphrase=wrong", "--json"]);
    let payload = assert_json_error_contract(&restore_output, 1, "runtime");
    assert!(
        payload["error"]["message"]
            .as_str()
            .is_some_and(|message| message.contains("decryption failed"))
    );
}

#[test]
fn sync_restore_json_detects_conflict_after_local_change() {
    let env = TestEnv::new("sync-restore-conflict");
    let sync_dir = env.root.join("sync-bundle");
    let backup_arg = format!("--sync-backup={}", sync_dir.display());
    let restore_arg = format!("--sync-restore={}", sync_dir.display());
    ...
    let mutate_output = env.run(&["--goal=15,1", "--json"]);
    assert_eq!(mutate_output.status.code(), Some(0));
    assert!(stderr_text(&mutate_output).trim().is_empty());

    let restore_output = env.run(&[restore_arg.as_str(), "--sync-passphrase=secret", "--json"]);
    let payload = assert_json_error_contract(&restore_output, 1, "runtime");
    assert!(
        payload["error"]["message"]
            .as_str()
            .is_some_and(|message| message.contains("conflict detected"))
    );
}
