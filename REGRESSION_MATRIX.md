...
| v0.15.3 | Blocklist category config remains compatibility-only with config-doctor guidance, while profile-level blocklist/allowlist site management remains the supported path. | Blocking cleanup | `config_doctor_reports_deprecated_blocklist_categories` plus `parse_rejects_removed_blocklist_category_flags` |
