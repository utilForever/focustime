...
| v0.15.1 | Removed migration-window and encrypted sync flags stay unavailable and emit targeted JSON `error.hint` replacement guidance. | Removed command | `v015_removed_command_paths_keep_supported_json_guidance_only` |
...
| v0.15.8 | Deprecated weekday profile rule shims are removed: `--weekday-rules*` is no longer parsed, config migration drops `weekday_profile_rules`, and schedules/session templates remain the replacement path. | Schedule/config cleanup | `parse_weekday_rules_is_retired` plus `migrate_weekday_rules_removes_key_and_preserves_existing_triggers` |
...
| v0.15.3 | Blocklist category config remains deprecated with config-doctor migration guidance, while profile-level blocklist/allowlist site management remains the supported path. | Blocking cleanup | `config_doctor_reports_deprecated_blocklist_categories` plus `parse_rejects_removed_blocklist_category_flags` |
...
| v0.15.6 | Daemon local API lifecycle commands report retirement guidance while CLI timer/session/workflow commands and the TUI remain supported replacement workflows. | Integration cleanup | `daemon_lifecycle_json_emits_deprecated_replacement_payload` plus `daemon_lifecycle_json_contract_round_trip` |
...
| v0.15.6 | Runtime dependency removal candidates stay documented with owning daemon, calendar, and integration/WakaTime feature paths before cleanup removals change `Cargo.toml`. | Dependency cleanup | `v015_cleanup_docs_keep_matrix_and_release_guidance_aligned` |
| v0.15.7 | Standalone usage-signal CLI access stays removed as a plain unknown option while feature inventory remains the cleanup/reporting workflow. | Usage cleanup | `v015_removed_command_paths_keep_supported_json_guidance_only` plus `v015_removed_command_text_errors_keep_supported_replacement_guidance_only` |
...
| v0.15.8 | Deprecated blocklist category config migrates into profile-level `sites` and `allowlist_sites`, and category fields are not re-persisted. | Blocking cleanup | `config_migration_flattens_blocklist_categories_into_profile_rules` plus `normalize_merges_legacy_profile_lists_when_categories_exist` |
