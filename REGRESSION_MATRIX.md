...
| Temporary overrides | Break-glass uses the canonical `temporary_overrides` status/recovery model while temporary allowlist commands and runtime entries stay removed. | `app_restores_break_glass_override_from_canonical_snapshot`, `build_status_output_includes_break_glass_temporary_override`, `temporary_allowlist_add_json_command_is_removed` |
