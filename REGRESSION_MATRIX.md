...
| v0.15.2 | Backend-selected blocking preview details are available from `--diagnostics`, while standalone preview access stays removed with canonical replacement guidance. | Blocking diagnostics | `diagnostics_json_includes_blocking_preview_payload` plus `blocking_preview_json_reports_removed_command_guidance` |
...
| v0.15.7 | Standalone usage-signal CLI access stays removed and points cleanup/reporting workflows to feature inventory output. | Usage cleanup | `v015_removed_command_paths_keep_targeted_json_guidance` plus `v015_removed_command_text_errors_keep_the_same_replacement_guidance` |
