...
| Blocking diagnostics and site rules | Hosts-file blocking preview data is available through `--diagnostics`; setup checks report actionable hosts-file permission/readiness status; CLI site CRUD operates on one canonical blocklist while allowlist exceptions remain config/internal behavior and retired profile/category branching stays absent. | `diagnostics_json_includes_blocking_preview_payload`, `blocking_preview_json_reports_plain_unknown_option`, `blocklist_site_commands_collapse_existing_profiles_to_canonical_rules`, `site_manager_collapses_blocklist_profiles_into_canonical_rules`, `site_manager_uses_profile_sites_not_selected_category` |
...
| Retired blocking workflows | Break-glass, temporary allowlist, allowlist site-management commands, and blocklist profile command paths stay removed while supported canonical blocklist commands and config/internal allowlist rules remain available. | `parse_break_glass_commands_are_removed_with_guidance`, `usage_text_omits_break_glass_commands`, `parse_blocklist_profile_commands_are_retired`, `parse_allowlist_site_management_commands_are_removed_with_guidance`, `temporary_allowlist_add_json_command_is_removed`, `site_manager_renders_allowlist_mode_labels` |
...
| Artifacts | Stats export creates target directories and preserves JSON path/error contracts while backup/restore CLI workflows stay removed with manual file-copy guidance. | `parse_backup_and_restore_commands_are_removed_with_guidance`, `backup_and_restore_json_commands_are_retired_with_guidance`, `export_json_creates_target_dirs_and_preserves_path_fields`, `export_json_reports_target_directory_errors` |
| Integration runtime | Daemon local API lifecycle paths and retired WakaTime heartbeat paths remain removed; CLI timer/session/workflow commands and the TUI remain the supported automation/runtime surface. | `parse_rejects_retired_daemon_lifecycle_options`, `retired_daemon_lifecycle_commands_emit_json_usage_errors`, `usage_text_keeps_supported_cli_automation_replacements`, `config_migration_removes_retired_wakatime_sections`, `save_with_env_omits_retired_wakatime_sections`, `cargo test --all` |
| Dependency ownership | Runtime HTTP and Basic auth have no direct manifest owner after WakaTime retirement, while removed daemon paths do not reintroduce direct runtime ownership and retired calendar timezone parsing stays absent. | `Cargo.toml`, `Cargo.lock`, plus `rg -n "chrono_tz\|chrono-tz" src tests Cargo.toml Cargo.lock` and `rg -n "ureq\|wakatime" src tests Cargo.toml` during dependency cleanup |
