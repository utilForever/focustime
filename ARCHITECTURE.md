...
| `cli.rs` + `cli/*`              | CLI contract and execution pipeline split into `args`, `parsing`, `execute`, `status`, and `output`, including headless timer/session/workflow controls, temporary override controls, and local backup/restore. The standalone calendar refresh, feature inventory export, and daemon local API lifecycle commands are retired.                    | `app`, `config`, `stats`, `blocker`                                     |
...
| `schedule.rs`                   | Recurring schedule compile and conflict/occurrence logic                                                                                                                                                                                                                                        | `app`, `cli`, `config`                                                                        |
...
- For release docs updates, keep architecture-facing terminology aligned with the
  ...
  and release tag examples in contributor-facing docs (for example, `v0.16.1`
