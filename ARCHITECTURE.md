...
| `cli.rs` + `cli/*`              | CLI contract and execution pipeline split into `args`, `parsing`, `execute`, `status`, and `output`, including headless timer/session/workflow controls, temporary override controls, and local backup/restore. The standalone calendar refresh, feature inventory export, task metadata/goal, session template, and daemon local API lifecycle commands are retired.                    | `app`, `config`, `stats`, `blocker`                                     |
