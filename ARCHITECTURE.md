...
| `cli.rs` + `cli/*`              | CLI contract and execution pipeline split into `args`, `parsing`, `execute`, `status`, and `output`, including headless `--start`, schedule delay/break-glass workflow controls, and backup/restore data-file workflows                              | `app`, `config`, `stats`, `blocker`                                            |
