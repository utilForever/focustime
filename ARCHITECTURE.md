...
| `cli.rs` + `cli/*`              | CLI contract and execution pipeline split into `args`, `parsing`, `execute`, `status`, and `output`, including headless timer controls, daemon lifecycle commands, schedule delay/break-glass workflow controls, local backup/restore, and calendar ICS cache refresh                    | `app`, `daemon`, `config`, `stats`, `calendar`, `blocker`                                     |
...
- For release docs updates, keep architecture-facing terminology aligned with the
  ...
  and release tag examples in contributor-facing docs (for example, `v0.15.3`
